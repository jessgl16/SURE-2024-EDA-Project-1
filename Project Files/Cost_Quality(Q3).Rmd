...
```{r}
hospitals |> 
  ggplot(aes(y = heart_failure_cost, x = fct_reorder(heart_failure_quality, heart_failure_cost, .fun = median))) +
  geom_boxplot(aes(fill = heart_failure_quality), outlier.shape = NA) +
  scale_fill_brewer(palette = "Set3") +
  stat_summary(fun = mean, geom = "text", aes(label = round(..y.., 1)), color = "black", vjust = -0.5) +
  labs(
    title = "Heart Failure Cost by Quality",
    x = "Heart Failure Quality",
    y = "Heart Failure Cost",
    subtitle = "Data from CORGIS Dataset Project",
    captions = "*All ratings are compared to the national average"
  ) +
  theme_minimal() +
  theme(
